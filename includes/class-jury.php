...
final class Jury {
	...
	public static function hooks(): void {
		...
		add_action( 'admin_enqueue_scripts', static function () {
			...
			if ( $screen && 'mp_edition' === $screen->post_type && 'post' === $screen->base ) {
				wp_enqueue_script( 'mp-jury', plugins_url( '../assets/jury.js', __FILE__ ), array(), '0.19.0-beta.3', true );
				wp_enqueue_style( 'mp-jury', plugins_url( '../assets/jury.css', __FILE__ ), array(), '0.19.0-beta.3' );
			}
		} );
	}
	...
	public static function administrators( int $edition ): array {
		return array_filter( self::members( $edition ), static fn( $member ) => 'administrator' === $member['kind'] );
	}
	...
	public static function box( \WP_Post $post ): void {
		...
		echo '<p class="description">Enregistrez l’édition pour appliquer les changements. Un nouveau compte reçoit une invitation pour choisir son mot de passe ; la connexion se fait ensuite avec son email et son mot de passe. Pour un compte existant, cochez « Envoyer une invitation » si nécessaire ; son mot de passe est conservé. Un email doit apparaître dans un seul tableau.</p>';
		echo '<p class="description">Décocher « Actif » retire la personne des notifications et des votes de cette édition : ses anciennes notes sont conservées mais exclues du total. Les droits de gestion d’un administrateur restent attachés à son compte ; leur retrait se fait dans les comptes WordPress par un administrateur du site. Les autres droits d’un compte existant sont conservés.</p>';
	}
	private static function member_table( array $data, string $group, string $kind, string $button ): void {
		echo '<div class="mp-jury-group" data-group="' . esc_attr( $group ) . '"><div class="mp-jury-scroll" role="region" aria-label="' . esc_attr( 'administrator' === $kind ? 'Administrateurs du marché' : 'Votants pour la sélection' ) . '" tabindex="0"><table class="widefat mp-jury-members" id="mp-jury-' . esc_attr( $group ) . '"><thead><tr><th>Nom</th><th>Email</th><th>Participation</th><th>Invitation</th></tr></thead><tbody>';
		$members = array_filter( $data['members'], static fn( $member ) => $member['kind'] === $kind );
		foreach ( $members as $uid => $member ) { self::member_row( (string) $uid, $member, $uid, $group ); }
		if ( ! $members && 'administrator' === $kind ) { self::member_row( 'new_initial', array( 'name' => '', 'active' => true ), 0, $group ); }
		echo '</tbody></table></div><p><button type="button" class="button mp-jury-add">' . esc_html( $button ) . '</button></p><template>';
		self::member_row( '__INDEX__', array( 'name' => '', 'active' => true ), 0, $group );
		echo '</template></div>';
	}
	private static function member_row( string $index, array $member, int $uid, string $group ): void {
		$user = $uid ? get_userdata( $uid ) : false;
		$prefix = 'mp_jury[' . $group . '][' . $index . ']';
		...
		if ( 'members' === $group && $user && user_can( $user, 'mp_manage_editions' ) ) { echo '<br><small>Ce compte conserve ses droits d’administration du marché.</small>'; }
		echo '</td>';
		...
		echo ' <button type="button" class="button-link mp-jury-move">' . ( 'members' === $group ? 'Passer dans le tableau administrateur' : 'Passer dans le tableau votant' ) . '</button>';
		echo '</td></tr>';
	}
	...
	public static function configure( int $edition, array $raw ): true|\WP_Error {
		...
		foreach ( array( 'administrators' => 'administrator', 'members' => 'voter' ) as $group => $kind ) {
			foreach ( $raw[ $group ] ?? array() as $row ) {
				...
				if ( '' === $name || strlen( $name ) > 480 || ! is_email( $email ) || isset( $emails[ strtolower( $email ) ] ) ) { return new \WP_Error( 'invalid', 'Indiquez un nom et un email valide pour chaque personne. Un email doit apparaître dans un seul tableau ; utilisez le bouton pour déplacer une personne.' ); }
				...
				$rows[] = array( 'user_id' => $uid, 'email' => $email, 'name' => $name, 'kind' => $kind, 'active' => '1' === ( $row['active'] ?? '0' ), 'invite' => '1' === ( $row['invite'] ?? '0' ) );
			}
		}
