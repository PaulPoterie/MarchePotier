# Administration du marché et votes

Guide du fonctionnement actuel de la branche `feature/votes-organisateurs`. Pour la structure du code, les données et les commandes de vérification, voir [Développement](DEVELOPPEMENT.md).
...
6. Dans **Gestion des candidatures → Examiner**, chaque personne choisit sa note entière entre **0 et 5** dans **Ma note**, puis clique sur **Valider ma note**. Le tableau **Notes du jury** présente les autres notes en lecture seule. Les notes des autres personnes sont actualisées au chargement de la page.
...
- La suite couvre l’administrateur unique, les invitations, les droits, les votes simultanés et hors période, les blocs, le suivi des votes, les filtres personnels et les tris. Le nombre de vérifications réussies est affiché à chaque exécution ; conserver le résultat daté avec la livraison.
...
- Les ZIP du dossier `dist` sont des instantanés de livraison, pas le reflet automatique de la branche. Aucun ZIP n’est généré à chaque correction ; vérifier son contenu avant installation.
